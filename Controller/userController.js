...
const registerAUser = async (req, res) => {
    try {
        ...
        if (!req.body.firstName) {
            return res.status(200).send({ status: false, message: "Please provide First Name" })
        }

        if (!req.body.lastName) {
            return res.status(200).send({ status: false, message: "Please provide Last Name" })
        }

        if (!req.body.mobile) {
            return res.status(200).send({ status: false, message: "Please provide Mobile" })
        }

        if (!req.body.email) {
            return res.status(200).send({ status: false, message: "Please provide email" })
        }

        if (!req.body.message) {
            return res.status(200).send({ status: false, message: "Please provide message" })
        }

        if (!req.body.dataType) {
            return res.status(200).send({ status: false, message: "Please provide Data Type" })
        }

        const findUser = await User.findOne({ email: email })
        const usedNuber = await User.findOne({ mobile: req.body.mobile })
        if (usedNuber) {
            return res.status(200).send({ status: false, message: "Mobile already used, try another Number" })
        }
        ...
        if (findUser) {
            ...
            if (!isValid(email)) return res.status(200).send({ status: false, message: "please enter email in string format" })
            if (!/[a-z0-9]+@[a-z]+\.[a-z]{2,3}/.test(email)) return res.status(200).send({ status: false, message: "please enter valid email" })


            let msg = "Thanks , we have appreceate that you have taken time to write us <br/>[Dumping Data]"
            const data = {
                to: req.body.email,
                from: 'dumpingdata@gmail.com',
                subject: "Registration Success",
                text: "Thanks , we have appreceate that you have taken time to write us <br/>[Dumping Data]",
                html: msg
            }
